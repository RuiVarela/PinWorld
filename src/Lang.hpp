...
namespace pw {
	...
	template<typename T> inline T fract(T const value) { return value - floor(value); }
	template<typename T> inline T smoothstep(T const edge0, T const edge1, T x) {
	...
	//
	template<typename T> inline T repeatSpace(T const& half_range, T const& value) {
